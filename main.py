...
#    util.plotLearningCurve('performance/simpleNet_performance')
    ...
    for b in range(5):
        ...
        for a in range(10):
            ...
            for d in range(10):
                ...
                policyCartPole.train(env, policy, noise=True, printUpdate = False, n_episodes=500, save_model= 50)
                
                perfs.append(util.evaluateFinalCartPole(env, policy, noise=True))
                print('                          {:3.2f}'.format(perfs[-1]))
                
                save_path = ('performance/simpleNet_b{}_a{:.6f}_df{:.3f}'.format(buff,alpha,d_factor))
                util.learningCurve(env, policy, noise=True, save_path=save_path, save_model_n=50, n_episodes=500)
    ...
    np.save('performance/hyperParamTune_episode500_saveModel50', final)
